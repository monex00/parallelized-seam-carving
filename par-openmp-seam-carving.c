...
static void compute_seam(Mat dp, int *seam)
{
    ...
    #pragma omp parallel
    {
        float local_min = MAT_AT(dp, y, 0); // local copy of the minimum
        int local_min_idx = 0; // local copy of the index of the minimum
        
        #pragma omp for
        for (int x = 1; x < dp.width; ++x) { // every thread computes its own local minimum
            if (MAT_AT(dp, y, x) < local_min) {
                local_min = MAT_AT(dp, y, x);
                local_min_idx = x;
            }
        }
        ...
        #pragma omp critical
        {
            if (local_min < MAT_AT(dp, y, seam[y])) {
                seam[y] = local_min_idx;
            }
        }
    }

    // This part cannot be parallelized due to dependencies
...
static void markout_sobel_patches(Mat grad, int *seam)
{
    #pragma omp parallel for
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < seams_to_remove; ++i) {
        ...
        for (int cy = 0; cy < grad.height; ++cy) {
            for (int cx = 0; cx < grad.width; ++cx) {
                if (cx >= seam[cy] && *(uint32_t*)&MAT_AT(grad, cy, cx) == 0xFFFFFFFF) {
                    MAT_AT(grad, cy, cx) = sobel_filter_at(lum, cx, cy);
                }
                else if (cx < seam[cy] && *(uint32_t*)&MAT_AT(grad, cy, cx) == 0xFFFFFFFF) {
                    MAT_AT(grad, cy, cx) = sobel_filter_at(lum, cx, cy);
                }
            }
        }
